/*
@@程式代號 = DOCRM820M_Controller.java
@@程式名稱 = 系統別Controller
@@程式版本 = V1.000
@@更新日期 = 2016/11/23
@@檢查碼  = 內容由YPM自動產生
...
@Controller
@Scope("prototype")
@RequestMapping("/docrm/DOCRM820M_Controller.do")
public class DOCRM820M_Controller extends DOCRMController {

	@Autowired
	private DOCRMUtilManager		docrmUtilManager;
	
    @Autowired
    private DOCRM820M_Manager docrm820M_Manager;
    ...
    public void processQuery(RequestBean requestBean, ResponseBean responseBean,
        ...
        Map<String, ?> dataMap = docrm820M_Manager.processQuery(paramMap);
    ...
    public void processAddSave(RequestBean requestBean, ResponseBean responseBean,
            HttpServletResponse response) throws Exception {
        Map paramMap = requestBean.getRequestMap();
        String DOCRM_TP = paramMap.get("DOCRM_TP").toString();
		String DOCRM_TP2 = paramMap.get("DOCRM_TP2").toString();

		if (!DOCRM_TP.equals(DOCRM_TP2)) {
			Map queryMap = new HashMap();
			queryMap.put("DOCRM_TP", DOCRM_TP);
			Map resultMap = (Map) docrmUtilManager.processSelectTableByAny("DOCRMT820", queryMap, false)
					.getFirstByMap();

			if (resultMap != null) {
				throw new LTSApplicationException("已有相同的代碼");
			}
		}
		
		
        Map<String, ?> dataMap = docrm820M_Manager.processAddSave(paramMap);
        responseBean.setData(dataMap);
    ...
    public void processEdit(RequestBean requestBean, ResponseBean responseBean,
        ...
        String TAX_TP =
    			HtmlTagFactory.renderOptionHtml("TAX_TP", docrmUtilManager.processDDL("TAX_TP"), "@{PHRASE_TITLE}",
    				"@{PHRASE_KEY}");
        Map<String, Object> rtn = new HashMap<String, Object>();
        Map<String, Object> dataMap = docrm820M_Manager.processEdit(paramMap);
    ...
    public void processEditSave(RequestBean requestBean, ResponseBean responseBean, HttpServletResponse response)
		...
		if (!DOCRM_TP.equals(DOCRM_TP2)) {
			...
			if (resultMap != null) {
				throw new LTSApplicationException("已有相同的表單種類");
			}
		}
        


        Map<String, ?> dataMap = docrm820M_Manager.processEditSave(paramMap);
        responseBean.setData(dataMap);
    }
    ...
    public void processDelete(RequestBean requestBean, ResponseBean responseBean,
        ...
        Map<String, ?> dataMap = docrm820M_Manager.processDelete(paramMap);
